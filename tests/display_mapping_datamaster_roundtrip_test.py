...
db=ROOT/"data"/"_display_dm_roundtrip.db"; paths=(db,Path(str(db)+"-wal"),Path(str(db)+"-shm"))
for p in paths:
...
try:
    ...
    dm=DataMasterService(s,Runtime()); report=dm.parse("current.xlsx",dm.export_current())
    assert report["valid"],report["errors"]
    mapping=json.loads(report["data"]["parameters"][0]["display_value_mapping_json"])
    assert mapping=={"0":"无","1":"有"},mapping
finally:
    for p in paths:
        if p.exists(): p.unlink()
print("PASS DataMaster display mapping round-trip")
