# -*- coding: utf-8 -*-
"""Emergency fallback must not bypass feasible explicit-anchor invariants."""
...
import app.local_generator as lg  # noqa: E402
...
def main():
    ...
    ], "indicator_filter_mode": "all", "min_capability": 50, "target_protocol": None}
    bounds = filters_to_anchors(request["indicator_filters"], PARAMETER_DEFINITIONS, "all")
    tag_map = {}

    record, base = gen._emergency_candidate(
        SEEDS, bounds, PARAMETER_DEFINITIONS, request, tag_map, None,
        budget={"attempted": 0, "max": 5},
    )
    assert record is not None
    assert record["params"]["weight"] <= 4, record["params"]

    # If invariant check reports a violation, emergency must reject the candidate.
    original = lg.validate_anchor_integrity
    lg.validate_anchor_integrity = lambda *a, **k: [{"parameter_id": "weight", "requested": "<=4", "actual": 5.0}]
    try:
        blocked, _ = gen._emergency_candidate(
            SEEDS, bounds, PARAMETER_DEFINITIONS, request, tag_map, None,
            budget={"attempted": 0, "max": 5},
        )
        assert blocked is None, "emergency must reject anchor-invariant violations"
    finally:
        lg.validate_anchor_integrity = original

    print(json.dumps({"status": "PASS", "message": "emergency fallback 受 anchor invariant 约束"}, ensure_ascii=False))
