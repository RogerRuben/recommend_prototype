...
def write_workbook_bytes(sheets):
    ...
    workbook_xml = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        '<workbook xmlns="%s" xmlns:r="%s"><sheets>%s</sheets></workbook>') % (_MAIN, _REL, ''.join(workbook_sheets))
    ...
    with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for i, (_, rows) in enumerate(sheets, 1):
            ...
            sheet_xml = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
                ...
                '<cols>%s</cols><sheetData>%s</sheetData><autoFilter ref="A1:%s%d"/></worksheet>') % (_MAIN, cols, ''.join(row_xml), column_name(max_cols - 1), max(len(rows), 1))
