...
页面支持按名称搜索、按历史/专家/其他正式方案筛选、全选或反选当前结果，并限制每次最多分析 30 个方案。正常情况下每个模型只调用一次批量接口；若旧方案缺少模型必填字段，工作台会隔离失败方案，其余方案继续完成计算。完整配置、API 与只读边界见 [效费比分析工作台](docs/COST_EFFECTIVENESS_ANALYSIS.md)。
