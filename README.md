...
V22 新增独立的“效费比分析工作台”，默认运行于 `http://127.0.0.1:17000`。工作台以只读方式加载推荐数据库中的历史方案与专家保存方案，批量调用价格服务 `:18101` 和效能服务 `:18102`，提供效费比、Pareto 前沿、五项 KPI、方案排名、基准差异和明细对比。它不调用推荐系统 `:17891` 的业务 API，也不写入推荐数据库。
...
勾选“生成时保持不变”的属性始终保留每条历史种子的原始业务值，其优先级高于同字段需求锚点。若某条旧历史方案根本没有该属性，该种子会以 `FrozenParameterMissingError` 明确排除，不会由 Preflight、模型默认值或协议复评补出一个新值。模型评价返回的规范化参数只保留在评价审计中，不得反向覆盖生成方案参数。
...
维护工作簿上传失败时，前端会区分文件读取失败、读取中止、非 JSON 服务响应和后端业务校验错误，并显示可操作的中文原因；不再把底层 `FileReader` 事件渲染成 `undefined`。
